...
function ApplicationsPage() {
  ...
  return (
    <div className="page-stack">
      <div className="toolbar"><div /><Button type="primary" icon={<Plus size={16} />} onClick={() => { setSelected(null); form.resetFields(); form.setFieldsValue({ type: 'oidc_client', status: 'active', redirect_uris: [], allowed_scopes: ['openid', 'profile'], grant_types: ['authorization_code'], response_types: ['code'], pkce_required: true }); setOpen(true); }}>{t('common.create')}</Button></div>
      ...
      <Modal title={selected ? t('common.edit') : t('common.create')} open={open} onOk={save} onCancel={() => setOpen(false)}>
        <Form form={form} layout="vertical"><Form.Item name="name" label={t('applications.name')} rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="type" label={t('applications.type')}><Select disabled={Boolean(selected)} options={APPLICATION_TYPES.map((value) => ({ value, label: t(`applications.type.${value}`, value) }))} /></Form.Item><Form.Item name="status" label={t('applications.status')}><Select options={['active', 'disabled'].map((value) => ({ value, label: t(`applications.status.${value}`, value) }))} /></Form.Item><Form.Item noStyle shouldUpdate={(prev, next) => prev.type !== next.type}>{() => form.getFieldValue('type') === 'oidc_client' && !selected ? <><Form.Item name="client_id" label={t('applications.clientId')}><Input placeholder={t('applications.autoWhenEmpty')} /></Form.Item><Form.Item name="redirect_uris" label={t('applications.redirectUris')}><Select mode="tags" /></Form.Item><Form.Item name="allowed_scopes" label={t('applications.scopes')}><Select mode="tags" /></Form.Item><Form.Item name="grant_types" label={t('applications.grantTypes')}><Select mode="tags" /></Form.Item><Form.Item name="response_types" label={t('applications.responseTypes')}><Select mode="tags" /></Form.Item><Form.Item name="pkce_required" label={t('applications.pkce')} valuePropName="checked"><Switch /></Form.Item></> : null}</Form.Item></Form>
      </Modal>
