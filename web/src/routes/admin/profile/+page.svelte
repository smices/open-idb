...
<script lang="ts">
  ...
  const saveProfile = async () => {
    ...
    try {
      const next = await api.updateAdminMe({ display_name: displayName.trim() });
      authUser.update((current) => current ? { ...current, display_name: next.display_name } : current);
  ...
  const savePassword = async () => {
    ...
    try {
      await api.updateAdminPassword({ current_password: currentPassword, new_password: newPassword });
...
<section class="grid gap-4 xl:grid-cols-2">
  ...
  <form class="card bg-surface-50-950 border border-surface-200-800 p-4 space-y-3" on:submit|preventDefault={saveProfile}>
  ...
  <form id="password" class="card bg-surface-50-950 border border-surface-200-800 p-4 space-y-3" on:submit|preventDefault={savePassword}>
    <div>
    ...
    {#if passwordError}
      <aside class="alert preset-tonal-error" role="alert"><p>{passwordError}</p></aside>
    {/if}
    ...
    <button class="btn preset-filled-primary-500" type="submit" disabled={passwordSubmitting}>
